...
_active_runs: dict[int, str] = {}  # run_id -> section_code
_stopped_runs: set[int] = set()
_stopped_sections: set[str] = set()
...
def clear_stop(run_id: int | None = None, section_code: str | None = None) -> None:
    """Clear stop requests for a run or section (e.g. when starting a retry)."""
    with _lock:
        if run_id is not None:
            _stopped_runs.discard(run_id)
        if section_code is not None:
            _stopped_sections.discard(section_code)


def register_run(run_id: int, section_code: str) -> None:
    """Register an active run."""
    with _lock:
        _active_runs[run_id] = section_code
        _run_processes.setdefault(run_id, set())
    set_current_run_id(run_id)
    logger.info("job_control: Registered active run_id=%d for section=%s", run_id, section_code)
...
def request_stop_section(section_code: str) -> list[int]:
    """
    Request cancellation of all active runs for the given section_code.
    Returns the list of run_ids that were marked stopped.
    """
    with _lock:
        _stopped_sections.add(section_code)
        matched_runs = [
            r_id for r_id, sec in _active_runs.items() if sec == section_code
        ]
        for r_id in matched_runs:
            _stopped_runs.add(r_id)
            terminate_processes_for_run(r_id)
    logger.info("job_control: Requested stop for section=%s, affected run_ids=%s", section_code, matched_runs)
    return matched_runs
...
def is_stop_requested(run_id: int | None = None, section_code: str | None = None) -> bool:
    """Check if cancellation has been requested for this run, section, or globally."""
    if _stop_all_flag:
        return True
    with _lock:
        if run_id is not None and run_id in _stopped_runs:
            return True
        if section_code is not None and section_code in _stopped_sections:
            return True
        if run_id is not None and run_id in _active_runs:
            sec = _active_runs[run_id]
            if sec in _stopped_sections:
                return True
    return False


def check_stop(run_id: int | None = None, section_code: str | None = None) -> None:
    """
    ...
    if is_stop_requested(run_id, section_code):
        raise JobStoppedError(
            f"Job execution cancelled by stop request (run_id={run_id}, section={section_code})"
        )
